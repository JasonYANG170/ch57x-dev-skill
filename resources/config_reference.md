...
Each BLE project has a `config.h` file (typically in `HAL/include/` or `APP/include/`) that controls the BLE stack behavior. All macros below are `#define` directives.
